...
| Submit approval decision | `POST /api/v1/workflows/{workflow_id}/approvals` on gateway-api | Requires `X-Actor-ID`; workflow must be reviewable | `workflow:review_decide` | Should call policy-engine and emit audit event before/after decision dispatch |
| Create replay run | `POST /api/v1/workflows/{workflow_id}/replay-runs` on gateway-api | Requires `X-Actor-ID`; replay is side-effect free | `workflow:replay_create` | Replay remains privileged because it inspects operational history |
...
| Create recovery action | `POST /api/v1/workflows/{workflow_id}/recovery-actions` on gateway-api | Requires `X-Actor-ID`, reason, and supported action | `workflow:recovery_execute` | Should require elevated operator/admin role and audit record |
| Retry outbox event | Recovery action `retry_outbox_event` | Requires explicit `workflow_event_outbox` target | `events:outbox_retry` | Must continue to use existing event publisher boundary |
| Dead-letter outbox event | Recovery action `mark_outbox_event_dead_lettered` | Requires explicit `workflow_event_outbox` target | `events:outbox_dead_letter` | Must be audit-visible because it suppresses future publishing attempts |
| Reconcile workflow projection | Recovery action `reconcile_workflow_projection` | Gateway request record; workflow-engine owns mutation | `workflow:projection_reconcile` | Policy must preserve workflow-engine state ownership |
...
Current gap:
- `X-Actor-ID` proves local actor attribution, not authorization.
- there is no local role parser, permission model, policy decision boundary, or audit emission boundary yet.
